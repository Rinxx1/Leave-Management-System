...
<?php
	...
	$leavetypeid = $_GET['leavetypeid'];
	$leavetypeid2 = $_GET['leaveid'];
	$appliedcred = "";

	$getcreds = "SELECT * FROM `tblempcred` INNER JOIN tblleaves ON tblleaves.LeaveType = tblempcred.leaveid WHERE tblempcred.leaveid = '$leavetypeid' AND tblempcred.emp_id = '$employeeid' AND tblleaves.id = '$leavetypeid2'  ";
	$resg = mysqli_query($conn, $getcreds);
	$rowgetc = mysqli_fetch_array($resg);

	$finaltotal = $rowgetc['credits'] - $rowgetc['num_days'];

	// echo "<script>alert('".$rowgetc['credits']."');</script>";
	...
	{ 
		...
		elseif ($status === '1') {
				...
				// $getcreds = "SELECT * FROM tblempcred WHERE emp_id = '$employeeid' AND $leaveid = '$leavetypeid' ";



				// $getcred = "UPDATE tblempcred SET ";


				$result = "UPDATE tblleaves, tblemployees set tblleaves.registra_remarks='$description',tblleaves.admin_status='$status',tblleaves.AdminRemarkDate='$admremarkdate', tblemployees.Av_leave='$REMLEAVE' where tblleaves.empid = tblemployees.emp_id AND tblleaves.id='$did'";

				

					$updatecred = "UPDATE tblempcred SET credits = '$finaltotal' WHERE emp_id = '$employeeid' AND leaveid = '$leavetypeid' ";

					// echo $rowgetc['credits'] > $rowgetc['num_days'];

					if($rowgetc['credits'] - $rowgetc['num_days']){

						if(mysqli_query($conn, $updatecred)){
						...
						echo "<script>alert('Credits iss not enough');</script>";
					}
				...
				<div class="pd-20 card-box mb-30">
					...
					<form method="post" action="">
						...
						{     
							...
							$appliedcred = $result->num_days;
						?>  

						<div class="row">
							<div class="col-md-4 col-sm-12">
								<div class="form-group">
									<label style="font-size:16px;"><b>Full Name <?php $rowgetc['credits']  ?></b></label>
						...
						<div class="row">
							...
							<div class="col-md-3">
								<div class="form-group">
									<label style="font-size:16px;"><b>HR Status</b></label>
